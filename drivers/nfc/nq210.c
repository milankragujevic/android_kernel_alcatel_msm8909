		...
		size_t count, loff_t *offset)
{
	...
	if (!gpio_get_value_cansleep(pn544_dev->irq_gpio)) {
		...
		ret = wait_event_interruptible(pn544_dev->read_wq,
				gpio_get_value_cansleep(pn544_dev->irq_gpio));
...
static int pn544_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   if (client->dev.of_node) {
	...
	//[Feature]-Add-BEGIN  comment out by qian.zhao to cancel dts analysis ,2016/02/02
    #if 0
    ...
    #endif
    //[Feature]-Add-END  comment out by qian.zhao to cancel dts analysis ,2016/02/02

	pn544_dev->firm_gpio = 905;
	pn544_dev->irq_gpio = 902;
	pn544_dev->ven_gpio = 906;
	...
	if (IS_ERR(bb_clk2)) {
		...
		//clk_set_rate(bb_clk2, NFC_RF_CLK_FREQ);
	...
	gpio_get_value_cansleep(pn544_dev->irq_gpio);
	gpio_get_value_cansleep(pn544_dev->firm_gpio);
