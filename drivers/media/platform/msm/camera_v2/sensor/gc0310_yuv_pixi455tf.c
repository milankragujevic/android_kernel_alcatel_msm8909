...
#define GC0310_SENSOR_NAME "gc0310_pixi455tf"
...
static const struct of_device_id gc0310_dt_match[] = {
	{.compatible = "e_welly,gc0310_pixi455tf", .data = &gc0310_s_ctrl},
	{}
};
	...
	void __user *argp)
{
	...
	/*copy from user the desired resoltuion*/
		...
		rc = gc0310_i2c_write_table(s_ctrl,
				&gc0310_recommend_settings[0],
				ARRAY_SIZE(gc0310_recommend_settings));
...
{ 
    ...
    if(module_id_value != 1){
		//ewelly module pin is hign
        pr_err("msm_sensor ewelly gc0310 module id  doesnot match module_id_value=%d\n",module_id_value);
		return -ENODEV;
    }
    pr_err(" msm_sensor ewelly gc0310 module id pin matched  module_id_value=%d\n",module_id_value);
}else{
	pr_err("msm_sensor_ewelly gc0310 module id pin is not valid\n");
	return -ENODEV;
} 
#endif
