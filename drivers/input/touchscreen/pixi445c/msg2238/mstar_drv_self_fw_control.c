...
extern struct msg22xx_virtualpsensor msg22xx_vps;
...
//#include "PIXI4-4.5-TF-TXD-0XC.206-20151008.h"
//modify by Taoming.Zhao for upgrade fw to V5.14[2015.11.17]
//#include "PIXI4-4.5-TF_TXD_2238_V0xC.207_20151117.h"
#include "PIXI4-4.5-TF_TXD_2238_V0xC.208_20151203.h"
...
u32 g_GestureWakeupMode[2] = {0x00000001, 0x00000000};	//modify by mike.li, only support double click.
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                if (g_EnableTpProximity && ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
                {
					...
					input_report_abs(msg22xx_vps.virtualdevice, ABS_DISTANCE, g_FaceClosingTp);
					input_sync(msg22xx_vps.virtualdevice);
//					msg22xx_tpd_proximity_last_faraway = msg22xx_tpd_proximity_faraway;
	...
	if (nUpdateBinMinor > nMinor)
	#endif
        {
            ...
            if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF)
            {
                ...
                g_IsUpdateFirmware = 0x11;
				/*The thread can easily be interrupted to upgrade the upgrade by ke.li@tcl.com [2015.10.21]  PR[1102452]   Start*/
				_DrvFwCtrlUpdateFirmwareBySwIdDoWork(NULL);
				//queue_work(_gUpdateFirmwareBySwIdWorkQueue, &_gUpdateFirmwareBySwIdWork);
				/*The thread can easily be interrupted to upgrade the upgrade by ke.li@tcl.com [2015.10.21]  PR[1102452]   End*/
                return;
            }
            else
            {
...
//    DBG("*** %s() ***\n", __func__);  // add for debug
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        else //touch on screen
        {
            ...
            {
#ifdef CONFIG_TP_HAVE_KEY
                if (tInfo.nTouchKeyCode == 2) // TOUCH_KEY_BACK
                {
                    nTouchKeyCode = g_TpVirtualKey[2];           
                }
                else if (tInfo.nTouchKeyCode == 4) // TOUCH_KEY_HOME
                {
