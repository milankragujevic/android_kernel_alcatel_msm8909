...
#include "ft6x06_ts.h"
#include <linux/wakelock.h> //wake lock
...
#if defined(CONFIG_TOUCHPANEL_PROXIMITY_SENSOR)||defined(CONFIG_FT6336_TOUCHPANEL_USB_DETECT)
 struct i2c_client  *ft_g_client=NULL;
#endif

#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
#include <linux/sensors.h>
 u8 tpd_proximity_ft6336_enable 			= 0;
 u8 tpd_proximity_ft6336_suspend 		= 0;
 u8 tpd_proximity_ft6336_far 		= 1;//0-->close ; 1--> far away
 u8 tpd_proximity_ft6336_last_far = 1;
...
extern struct sensors_classdev virtual_sensors_proximity_cdev;
extern  int virtual_psensor_input_register(struct i2c_client *pClient);
extern int vps_set_enable(struct sensors_classdev *sensors_cdev,unsigned int enable);
...
static irqreturn_t ft6x06_ts_interrupt(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&data->ps_wakelock, HZ/20);// add wake lock
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) 
		{
            if(y >= 900)
			{
				switch(x)
				{
				//Begin .Modified by  weigeng.huang@tcl.com  for fixing wrong x value while reporting BACK key and MENU key  in PIXI445TFVZW ,2015-10-19,task number:745729
				#if defined(TCT_PIXI445TF)
					case 80:
					...
					default:
					    ...
					    break;
				#elif (defined(JRD_PROJECT_PIXI3454GSPR) || defined(JRD_PROJECT_PIXI3554GEVDO) || defined(JRD_PROJECT_PIXI3554GEVDOGREATCALL))
					case 80:
						input_report_key(ip_dev, 158, 1);
						break;
					case 240:
						input_report_key(ip_dev, 172, 1);
						break;
					case 400:
					...
					default:
					    ...
					    break;
				#endif
				//End .Modified by weigeng.huang@tcl.com  for  fixing wrong x value while reporting BACK key and MENU key in PIXI445TFVZW ,2015-10-19,task number: 745729
				}
			}
			else{
...
int ft6x06_ts_resume(struct device *dev)
{
	struct ft6x06_ts_data *data = dev_get_drvdata(dev);
	int err = 0;
#ifdef FT6336_GESTURE_FUNCTION
			   ...
			   const struct i2c_device_id *id)
{
 ...
 if(err)
	...
	wake_lock_init(&data->ps_wakelock,WAKE_LOCK_SUSPEND, "ft6x06_input_wakelock");// add wake lock
...
unreg_inputdev:
	...
	input_dev = NULL;
free_inputdev:
	wake_lock_destroy(&data->ps_wakelock); //free wake lock
