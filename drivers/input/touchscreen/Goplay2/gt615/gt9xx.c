...
{	
	...
	if (Rbuf[5] == 0x00)
	{
		 num_read_chars = sprintf(buf, "GT_%c%c%c_%02x%02x_%02x_%s\n", Rbuf[2], Rbuf[3], Rbuf[4], Rbuf[7], Rbuf[6],cfg_buf[0],module);
	}
	else
	{
		 num_read_chars = sprintf(buf, "GT_%c%c%c%c_%02x%02x_%02x_%s\n", Rbuf[2], Rbuf[3], Rbuf[4], Rbuf[5], Rbuf[7], Rbuf[6],cfg_buf[0],module);
	}
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    //#else
	{
...
#if 0
       // Add by hong.lan@tcl.com start i2c resume comphete flag PR:538578  2015/09/06
       ...
       // Add by hong.lan@tcl.com end i2c resume comphete flag PR:538578  2015/09/06
#endif    
		msleep(25);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!vdd_ana) {
		...
		{
			rc = regulator_set_voltage(vdd_ana, GOODIX_VTG_MIN_UV,
					   GOODIX_VTG_MAX_UV);
			if (rc) {
				dev_err(&client->dev,
	...
	if (!vcc_i2c) {
		...
		{
			rc = regulator_set_voltage(vcc_i2c, GOODIX_I2C_VTG_MIN_UV,
...
ERR_SET_I2C:
	if (!IS_ERR(vcc_i2c))
		regulator_set_voltage(vcc_i2c, 0, GOODIX_I2C_VTG_MIN_UV);
ERR_SET_VCC:
	if(!IS_ERR(vdd_ana))
		regulator_set_voltage(vdd_ana, 0, GOODIX_VTG_MIN_UV);
/* [Feature] ADD-END by hong.lan@tcl.com for set voltage 2015/06/20 PR:359508 */
ERR_GET_VCC:
