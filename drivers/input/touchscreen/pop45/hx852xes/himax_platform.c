...
void himax_gpio_power_release(struct i2c_client *client,struct himax_i2c_platform_data *pdata)
{
	if (gpio_is_valid(pdata->gpio_irq))
		gpio_free(pdata->gpio_irq);
...
int himax_gpio_power_config(struct i2c_client *client,struct himax_i2c_platform_data *pdata)
{
	...
	if (pdata->gpio_reset >= 0) {
		...
		printk("%s:request rst pin:%d", __func__, pdata->gpio_reset);
		if (error < 0){
	...
	if (pdata->gpio_3v3_en >= 0) {
		...
		if (error) {
			E("unable to set gpio_3v3_en for gpio [%d]\n",
				pdata->gpio_3v3_en);
			goto err_gpio_3v3_en_req;
		}
	}
	if (gpio_is_valid(pdata->gpio_irq)) {
	/* configure touchscreen irq gpio */
