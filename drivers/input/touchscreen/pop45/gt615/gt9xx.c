...
int gtp_rst_gpio;
int gtp_int_gpio;
extern bool i2c_resume_completed; // Add by hong.lan@tcl.com i2c resume comphete flag PR:538578  2015/09/06

static int id_sensor=0; // [Feature] Add by hong.lan for store sensor id 2015/06/25

static bool request_gpio = false;

/* [Feature] ADD-BEGIN by hong.lan@tcl.com for set voltage 2015/06/20 PR:359508 */
#define GOODIX_VTG_MIN_UV	2600000
#define GOODIX_VTG_MAX_UV	3300000
#define GOODIX_I2C_VTG_MIN_UV	1800000
#define GOODIX_I2C_VTG_MAX_UV	1800000
/* [Feature] ADD-END by hong.lan@tcl.com for set voltage 2015/06/20 PR:359508 */

bool gt9xx_update_test = false;
...
extern u8 gup_manual_update_cfg_file(struct i2c_client *client, char *cfg_name);
extern u8 gup_manual_update_bin_file(struct i2c_client *client, char *cfg_name);
extern u8 gup_force_update_bin_file(struct i2c_client *client);

bool GTP_DEBUG_MODE_ON = false;
#define GTP_DEBUG(fmt,arg...)          do{\
                                         if(GTP_DEBUG_MODE_ON)\
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            ...
            GTP_INFO("Entering Gesture mode enabled.");
            return ret;
        }
        msleep(10);
    }
...
static ssize_t jrd_Debug_Mode_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
    printk("*** %s()***\n", __func__);
...
static ssize_t jrd_Debug_Mode_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	if( buf[0] == '0')
	{
	  GTP_DEBUG_MODE_ON  = false;
	}
	else if(buf[0]=='1')
	{
		GTP_DEBUG_MODE_ON  = true;
	}
	else
	{
	...
	printk("*** %s() GTP_DEBUG_ON = %d ***\n", __func__,GTP_DEBUG_MODE_ON);

    return size;

}

//[Frature] Add-Begin by hong.lan for touch panel version node RP:390414 2015/06/26
...
{	
	...
	if (Rbuf[5] == 0x00)
	{
		 num_read_chars = sprintf(buf, "GT%c%c%c_%s_FW%02x%02x_CF%02x\n", Rbuf[2], Rbuf[3], Rbuf[4], module, Rbuf[7], Rbuf[6],cfg_buf[0]);
	}
	else
	{
		 num_read_chars = sprintf(buf, "GT%c%c%c%c_%s_FW%02x%02x_CF%02x\n", Rbuf[2], Rbuf[3], Rbuf[4], Rbuf[5], module, Rbuf[7], Rbuf[6],cfg_buf[0]);
	}
...
static DEVICE_ATTR(fw_ver, 0644,
					gt9xx_tpfwver_show, gt9xx_tpfwver_store);

/* upgrade self config file
*  example: echo 1 > fw_update_test
*/
static ssize_t jrd_fw_update_test_show(struct device *dev,struct device_attribute *attr, char *buf)
{	
	/*
	int count = 0, i;
	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {0x80, 0x47};
	
    //count +=snprintf(buf + count, PAGE_SIZE - count,"Send cfg test:  [%s]\n", gt9xx_send_cfg_test? "true": "false");
  
	printk("==== GT9XX config real value====\n");
	
    gup_i2c_read(i2c_connect_client, temp_data, GTP_CONFIG_MAX_LENGTH + 2);
...
static ssize_t jrd_fw_update_test_store(struct device *dev,	struct device_attribute *attr,const char *buf, size_t count)
{	

	u8 config_data[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
	//char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {0x80, 0x47};
	...
	struct goodix_ts_data *ts;
	ts = i2c_get_clientdata(i2c_connect_client);
	GTP_INFO("** jrd_fw_update_test_store ** \n");
	gt9xx_update_test = false;
	
	GTP_INFO("****** ts->gtp_cfg_len = %d *****\n",ts->gtp_cfg_len);
	
	ret = gtp_parse_dt_cfg(&ts->client->dev, &config_data[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, id_sensor);
	if (ret < 0) {
...
*/
	for (retry = 0; retry < 5; retry++)
    {
        ret = gup_i2c_write(i2c_connect_client, config_data , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
        	GTP_INFO("update cfg success.");
            break;
        }
    }
	...
	printk("=========================\n");
	*/
	GTP_INFO("Start Force update bin.");
	ret = gup_force_update_bin_file(i2c_connect_client);
	
	if(SUCCESS == ret)
	{
		gt9xx_update_test = true;
		GTP_INFO("Update cfg and bin success.");
	}
	else
	{
		gt9xx_update_test = false;
		GTP_INFO("Update bin fail.");
	}
...
/* show update fw val
*  example: cat fw_manual_cfg_update
*/
static ssize_t jrd_fw_manual_send_cfg_update_show(struct device *dev,struct device_attribute *attr, char *buf)
{	
	int count = 0, i;
	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {0x80, 0x47};
...
/*  write fw to update
 *  example: echo *.cfg > fw_manual_cfg_update
 */
static ssize_t jrd_fw_manual_send_cfg_update_store(struct device *dev,	
		struct device_attribute *attr,
...
{	
	...
	GTP_INFO("** %s\n ,%s \n**",__func__,fwname);
	gup_manual_update_cfg_file(i2c_connect_client,fwname);
...
/* 
 * write fw to update
 * example: echo *.bin > fw_manual_bin_update
*/
static ssize_t jrd_fw_manual_send_bin_update_store(struct device *dev,	
		struct device_attribute *attr,
		const char *buf, size_t count)
{	
	char fwname[128];
	...
	GTP_INFO("** %s\n ,%s \n**",__func__,fwname);
	gup_manual_update_bin_file(i2c_connect_client,fwname);

	return count;
}


static DEVICE_ATTR(fw_manual_cfg_update, 0644,
					jrd_fw_manual_send_cfg_update_show, jrd_fw_manual_send_cfg_update_store); 

static DEVICE_ATTR(fw_manual_bin_update, 0644,
					NULL, jrd_fw_manual_send_bin_update_store); 

static DEVICE_ATTR(fw_update_test, 0644,
					jrd_fw_update_test_show, jrd_fw_update_test_store); 

static DEVICE_ATTR(gesture_switch, 0644,  
					jrd_gesture_switch_show, jrd_gesture_switch_store);  
static DEVICE_ATTR(tp_ic_info, 0644,  
					jrd_tp_ic_info_show, NULL);
static DEVICE_ATTR(rawdata, 0644,  
					jrd_read_rawdata_show, jrd_read_rawdata_store); 
static DEVICE_ATTR(rawdata_result, 0644,
					jrd_rawdata_result_show, jrd_rawdata_result_store); 
static DEVICE_ATTR(debug, 0644,
					jrd_Debug_Mode_show, jrd_Debug_Mode_store); 

static struct attribute *TP_sysfs_attrs[] = {
	&dev_attr_gesture_switch.attr,
	&dev_attr_tp_ic_info.attr,
	&dev_attr_rawdata.attr,
	&dev_attr_rawdata_result.attr,
	&dev_attr_debug.attr,
	&dev_attr_fw_ver.attr, //[Frature] Add by hong.lan for touch panel version node RP:390414 2015/06/26
	&dev_attr_fw_update_test.attr,
	&dev_attr_fw_manual_cfg_update.attr,
	&dev_attr_fw_manual_bin_update.attr,
	NULL,
};
...
{ 
	...
	sys_ts_data = ts;
	TP_ctrl_kobj = kobject_create_and_add("Goodix_ts", &ts->client->dev.kobj);
	if (!TP_ctrl_kobj){
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
	...
	// Add by hong.lan@tcl.com start i2c resume comphete flag PR:359508  2015/08/31
	...
	// Add by hong.lan@tcl.com end i2c resume comphete flag PR:359508  2015/08/31
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	id_sensor = sensor_id; // [Feature] Add by hong.lan for store sensor id 2015/06/25
	
	if(id_sensor == 0)
	{		
		GTP_INFO("This is belong to Dijing's module");     
	}
	else
	{		
		GTP_INFO("This is belong to Unknow module");
	}
...
//    u8 retry = 0;
    ...
    //GTP_DEBUG_FUNC();
  	GTP_INFO(" gtp_i2c_test --client addr : %x ***", client->addr);
  //  while(retry++ < 1)
    {
...
static int gtp_power_init(struct i2c_client *client)
{
	int ret,rc;
	struct goodix_ts_data *ts= i2c_get_clientdata(client);
	...
	if (!ts->vdd_ana) {
		...
		{
			rc = regulator_set_voltage(ts->vdd_ana, GOODIX_VTG_MIN_UV,
					   GOODIX_VTG_MAX_UV);
			if (rc) {
				dev_err(&client->dev,
	...
	if (!ts->vcc_i2c) {
		...
		{
			rc = regulator_set_voltage(ts->vcc_i2c, GOODIX_I2C_VTG_MIN_UV,
...
ERR_SET_VCC:
	if(!IS_ERR(ts->vdd_ana))
		regulator_set_voltage(ts->vdd_ana, 0, GOODIX_VTG_MIN_UV);
/* [Feature] ADD-END by hong.lan@tcl.com for set voltage 2015/06/20 PR:359508 */
ERR_GET_VCC:
...
static int gtp_power_release(struct i2c_client *client)
{
	struct goodix_ts_data *ts= i2c_get_clientdata(client);

	regulator_set_voltage(ts->vcc_i2c, 0, GOODIX_I2C_VTG_MIN_UV);
	regulator_set_voltage(ts->vdd_ana, 0, GOODIX_VTG_MIN_UV);
	regulator_put(ts->vcc_i2c);
	regulator_put(ts->vdd_ana);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on) {
		if(gtp_power_on)
		{
			GTP_ERROR("GTP already power on.");
			return -1;
		}
		...
		if(!gtp_power_on)
		{
			GTP_ERROR("GTP already power off.");
			return -1;
		}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // GTP_DEBUG_FUNC();
    ...
    if (ret < 0)
    {
        GTP_ERROR("GTP TEST: I2C communication ERROR!");
	goto err_i2c_test_fail;
    }
...
//#else
	{
	    ...
	    msleep(58);
		ret = gtp_power_switch(ts->client, SWITCH_OFF);
		if (ret < 0)
	    {
...
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK && ts) {
		blank = ev_data->data;
		if (*blank == FB_BLANK_UNBLANK  && flag==true) {
			GTP_INFO("Resume by fb notifier.");
			goodix_ts_resume(ts);
			flag = false;
		}
		else if (*blank == FB_BLANK_POWERDOWN && flag==false) {
