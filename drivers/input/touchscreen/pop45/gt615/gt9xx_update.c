...
#define GUP_SEARCH_FILE_TIMES       25
...
u8 searching_file = 0;
bool IS_Force_Bin_Update = false;
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    u32 fw_len = 0;
    s32 pid_cmp_len = 0;
	u16 force_update_vid[] = {0x2020,0x2028,0x2031};
    u16_tmp = fw_head->vid;
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
		...
		//Force update bin file
		if(IS_Force_Bin_Update)
		{
			 GTP_INFO("Force enter update.");
			 IS_Force_Bin_Update = false;
			return SUCCESS;
		}
		else{
			//Judge vid for update 
			for (i = 0; i < sizeof (force_update_vid) / sizeof(force_update_vid[0]); i++)
			{
				GTP_INFO("IC VID:%04x", update_msg.ic_fw_msg.vid);
				GTP_INFO("FORCE UPDATE VID:%04x", force_update_vid[i]);

				if (update_msg.ic_fw_msg.vid == force_update_vid[sizeof (force_update_vid) / sizeof(force_update_vid[0])-1]) 
				{
					GTP_INFO("FW is the newest,id = 0x%x.", update_msg.ic_fw_msg.vid);
					return FAIL;
				}
				if(update_msg.ic_fw_msg.vid == force_update_vid[i])
				{
					//GTP_INFO("Need enter update.");
	            			return SUCCESS;  //Delete for not need update!
				}
				else{
					 GTP_INFO("Don't meet the third condition.");
	       			 	 GTP_INFO("force_update_vid != Ic VID, update aborted!");
				}
...
static s8 gup_manual_send_config(struct i2c_client *client)
{
	s32 file_len = 0;
    s32 ret = 0;
    s32 i = 0,j = 0;
    ...
    GTP_INFO("[update_cfg]Read config from file.");
    ...
    while(i++ < 5)
    {
        ...
        if(ret > 0)
        {
        	printk("==== GT9XX send cfg value====\n");
			for (j = 0 ; j < GTP_CONFIG_MAX_LENGTH ; j++)
			{
				printk( "0x%02X ", file_config[j+2]);
            ...
            GTP_INFO("[update_cfg]Manual send config SUCCESS.");
            break;
        }
...
static void gup_search_config_file(s32 search_type,char *cfg_name)
{
	   s32 i = 0;
	   ...
	   sprintf(filepath1, "/sdcard/%s", cfg_name);
	   sprintf(filepath2, "/data/%s", cfg_name);
	   ...
	   {			
		   ...
		   {
			   GTP_DEBUG("Search for %s, %s for config update.(%d/%d)", filepath1, filepath1, i+1, GUP_SEARCH_FILE_TIMES);
			   pfile = filp_open(filepath1, O_RDONLY, 0);
			   if (IS_ERR(pfile))
			   {
				   pfile = filp_open(filepath2, O_RDONLY, 0);
				   if (!IS_ERR(pfile))
				   {
					   GTP_INFO("Cfg file: %s for config update.", filepath2);
			   ...
			   {
				   GTP_INFO("Cfg file: %s for config update.", filepath1);
				   got_file_flag |= CFG_FILE_READY;
				   update_msg.cfg_file = pfile;
			   }
...
static int get_bin_file_data(st_fw_head* fw_head)
{
    s32 i = 0;
    s32 fw_checksum = 0;
...
static int gup_search_bin_file(s32 search_type,st_fw_head* fw_head,char *bin_name)
{
	   s32 i = 0,ret = 0;
	   struct file *pfile = NULL;
	   ...
	   sprintf(filepath1, "/sdcard/%s", bin_name);
	   sprintf(filepath2, "/data/%s", bin_name);
	   ...
	   {			
			 ...
			 {
				 GTP_DEBUG("Search for %s, %s for fw update.(%d/%d)", filepath1, filepath2, i+1, GUP_SEARCH_FILE_TIMES);
				 pfile = filp_open(filepath1, O_RDONLY, 0);
				 if (IS_ERR(pfile))
				 {
					 pfile = filp_open(filepath2, O_RDONLY, 0);
					 if (!IS_ERR(pfile))
					 {
						 GTP_INFO("Bin file: %s for fw update.", filepath2);
				 ...
				 {
					 GTP_INFO("Bin file: %s for fw update.", filepath1);
					 got_file_flag |= BIN_FILE_READY;
					 update_msg.file = pfile;
					 break;
				 }
    ...
    GTP_INFO("Bin firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
    ...
    GTP_INFO("firmware checksum:%x", fw_checksum&0xFFFF);
    if(fw_checksum&0xFFFF)
    {
        GTP_ERROR("Illegal firmware file.");
        return FAIL;
...
u8 gup_manual_update_cfg_file(struct i2c_client *client, char *cfg_name)
{
		int ret;

		GTP_INFO("** %s\n **",__func__);
		update_msg.file = NULL;
	    gup_search_config_file(AUTO_SEARCH_CFG,cfg_name);
        if (got_file_flag & CFG_FILE_READY)
        {
            ret = gup_manual_send_config(i2c_connect_client);
            if(ret <= 0)
            {
		...
		{
			GTP_INFO("Can't find config file /sdcard/%s or /data/%s .\n",cfg_name,cfg_name);
		}
...
u8 gup_force_update_bin_file(struct i2c_client *client)
{
	int ret;
	st_fw_head fw_head;
	s32 update_ret = FAIL;
	...
	// force update bin 
	IS_Force_Bin_Update = true;

	GTP_INFO("** %s\n **",__func__);
	update_msg.file = NULL;
	ret = get_bin_file_data(&fw_head);
	...
	else
	{
    	...
    	GTP_INFO("THE NEW IC PID:%s", update_msg.ic_fw_msg.pid);
    	GTP_INFO("THE NEW IC VID:%04x", update_msg.ic_fw_msg.vid);
...
u8 gup_manual_update_bin_file(struct i2c_client *client, char *bin_name)
{
	int ret;
	st_fw_head fw_head;
    s32 update_ret = FAIL;
	...
	GTP_INFO("** %s\n **",__func__);
	update_msg.file = NULL;
    ret = gup_search_bin_file(AUTO_SEARCH_BIN,&fw_head,bin_name);
