...
static u8 g_proximity_en = 0;
	...
	static unsigned char i_CTPM_FW[]=
	{
		...
		//#include "firmware/Dijing/DJN_6410_LC_CM236_C02_2016-03-11_Draw.i"
		//#include "firmware/Dijing/DJN_6410_LC_CM236_C04_2016-03-18_1656.i"
		//#include "firmware/Dijing/DJN_6410_QT_CM236_C05_2016-04-12_Draw.i"
		#include "firmware/Dijing/DJN_6410_QT_CM236_C06_2016-04-21_1906.i"
...
static void fb_notify_resume_work(struct work_struct *work);//add by weigeng.huang for resume in work
...
static int i_update_FW(void)
{
	...
	struct version_info update_list[] = {{0xe4,0x11,0x02},{0xe4,0x11,0x04}};
	int list_len = 0;
	int i = 0;
	...
	/*if (( private_ts->vendor_fw_ver_H < i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
		...
		list_len = sizeof (update_list) / sizeof(update_list[0]);
		
/*
		if ( private_ts->vendor_fw_ver_H==update_list[list_len-1].fw_ver_H
				&& private_ts->vendor_fw_ver_L==update_list[list_len-1].fw_ver_L
				&& private_ts->vendor_config_ver==update_list[list_len-1].cfg_ver )
		{
			I(" %s: FW Version is newest!\n", __func__);
			return 0;
		}
*/
		if(private_ts->vendor_sensor_id == 0x11)
		{
			...
			{		 
				...
				if ( private_ts->vendor_fw_ver_H==update_list[i].fw_ver_H
					&& private_ts->vendor_fw_ver_L==update_list[i].fw_ver_L
					&& private_ts->vendor_config_ver==update_list[i].cfg_ver )
				{
	...
	size_t len, loff_t *pos)
{
	...
	if (ts_data->vendor_sensor_id == 0x12)
	{
		module = "BooyiTech";
	}
	else 	if (ts_data->vendor_sensor_id == 0x11)
	{
		module = "Dijing";
	}
	else
	...
	if(!HX_PROC_SEND_FLAG)
		{
			ret += sprintf(buf, "%s_%s_FW:%#x,%x_CFG:%#x\n", HIMAX852xes_NAME, module,
				ts_data->vendor_fw_ver_H, ts_data->vendor_fw_ver_L, ts_data->vendor_config_ver);
			HX_PROC_SEND_FLAG=1;
		}
	else
		HX_PROC_SEND_FLAG=0;
...
static int himax_chip_self_test(void)
{
	...
	i2c_himax_write(private_ts->client, 0x82,&cmdbuf[0], 0, DEFAULT_RETRY_CNT);		//paco add 0318
	msleep(50);
	...
	msleep(30);
		...
		i2c_himax_read(private_ts->client, 0xB1, valuebuf, 8, DEFAULT_RETRY_CNT);
                if (valuebuf[0]!=0x00&&valuebuf[1]!=0x00) 
                        break; 
                msleep(5); 
	}
...
static ssize_t himax_force_update_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
...
static ssize_t himax_force_update_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	char buf[16] = {0};
	unsigned char* ImageBuffer = i_CTPM_FW;
	...
	if (len >= 16)
	{
		I("%s: no command exceeds 16 chars.\n", __func__);
		return -EFAULT;
	}
	if (copy_from_user(buf, buff, len))
	{
...
static struct file_operations himax_proc_force_update_ops =
{
	.owner = THIS_MODULE,
	.read = himax_force_update_read,
	.write = himax_force_update_write,
};
...
static int himax_touch_proc_init(void)
{
	...
	himax_proc_force_update_file = proc_create(HIMAX_PROC_FORCE_UPDATE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		himax_touch_proc_dir, &himax_proc_force_update_ops);
	if(himax_proc_force_update_file == NULL)
	{
		E(" %s: proc force_update file create failed!\n", __func__);
	}

	return 0 ;
#ifdef HX_SMART_WAKEUP
	fail_14: remove_proc_entry( HIMAX_PROC_COVER_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_DOT_VIEW
	fail_13: remove_proc_entry( HIMAX_PROC_HITOUCH_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_HITOUCH
	fail_12: remove_proc_entry( HIMAX_PROC_SELF_TEST_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_SELF_TEST
	fail_11: remove_proc_entry( HIMAX_PROC_FLASH_DUMP_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_FLASH_DUMP
...
{
		remove_proc_entry( HIMAX_PROC_FORCE_UPDATE_FILE, himax_touch_proc_dir );
#ifdef HX_SMART_WAKEUP
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	I(" DT-%s:display-coords = (%d, %d)", __func__, pdata->screenWidth,
		pdata->screenHeight);
//add by weigeng.huang for i2c power control
	pdata->i2c_pull_up = of_property_read_bool(dt,"himax,i2c_pull_up");
	if (!pdata->i2c_pull_up) {
		I(" DT:himax,i2c_pull_up value is not config\n");
	}
//add by weigeng.huang for i2c power control
	...
	ts->resume_in_workqueue = of_property_read_bool(dt,"himax,resume-in-workqueue");//add by weigeng.huang 
...
//add by weigeng.huang@tcl.com for pintrcl 
static int Himax_ts_pinctrl_init(struct himax_ts_data *ts)
{
	int retval;
...
static int Himax_ts_pinctrl_select(struct himax_ts_data *ts,bool on)
{
...
#endif
//add by weigeng.huang@tcl.com for pinctrl 
	err = Himax_ts_pinctrl_init(ts);
	if (!err &&ts->ts_pinctrl) {
		err = Himax_ts_pinctrl_select(ts, 1);
		if (err < 0)
...
himax_gpio_power_config(ts->client, pdata);
	...
	INIT_WORK(&ts->fb_notify_work,fb_notify_resume_work);//resume in workqueue
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
err_ic_package_failed:
	himax_gpio_power_release(ts->client, pdata);
...
err_dt_platform_data_fail:
	...
	if (ts->ts_pinctrl) 
	{
		err = Himax_ts_pinctrl_select(ts, 0);
		if (err < 0)
...
if(HX_ON_HAND_SHAKING)//chip on hand shaking,wait hand shaking
{
	...
	atomic_set(&ts->suspend_mode, 1);
	ts->pre_finger_mask = 0;
//add by weigeng.huang for pinctrl
	if (ts->ts_pinctrl) {
			err = Himax_ts_pinctrl_select(ts,0);
			if (err < 0) {
...
#endif
	//Sense On
	i2c_himax_write_command(ts->client, HX_CMD_TSSON, HIMAX_I2C_RETRY_TIMES);
	msleep(30);
	i2c_himax_write_command(ts->client, HX_CMD_TSSLPOUT, HIMAX_I2C_RETRY_TIMES);
...
#if defined(CONFIG_FB)
//add by weigeng.huang for resume in work begin
static void fb_notify_resume_work(struct work_struct *work)
{
	struct himax_ts_data * himax_data = container_of(work,struct himax_ts_data,fb_notify_work);
	himax852xes_resume(&himax_data->client->dev);
	printk(KERN_INFO"[%s] out\n",__func__);
}
//add by weigeng.huang for resume in work end
static int fb_notifier_callback(struct notifier_block *self,
				 unsigned long event, void *data)
{
	...
	I(" %s\n", __func__);
//modify by wiegand for resume in work begin 
	if (evdata && evdata->data && ts &&
			ts->client) {
		if(ts->resume_in_workqueue)
		{
//modify by wiegand weigeng.huang@tcl.com. Fb_set_suspend will not set blank value,if compare *blank first,may cause ramdump
			blank = evdata->data;
			...
			{
				if((*blank == FB_BLANK_POWERDOWN)||(*blank == FB_BLANK_HSYNC_SUSPEND)||(*blank == FB_BLANK_VSYNC_SUSPEND)||(*blank == FB_BLANK_NORMAL))
				{
					flush_work(&ts->fb_notify_work);
					himax852xes_suspend(&ts->client->dev);	
				}
			}
//modify by wiegand weigeng.huang@tcl.com. Should compare event first,
		...
		{    	
			...
			{	
				...
				break;
				default:
					printk(KERN_INFO "Not support fb_blank\n");
				break;
				}
			}		
		}
	}
//modify by wiegand for resume in work end	
	return 0;
}
#elif defined(CONFIG_HAS_EARLYSUSPEND)
static void himax_ts_early_suspend(struct early_suspend *h)
...
static int __init himax852xes_init(void)
{
	...
	rc = tpd_driver_add(&tpd_device_driver);
	TPD_DMESG("%s:[Himax] tpd_driver_add  rc = %d\n", __func__,rc);	
#endif
#ifdef QCT
