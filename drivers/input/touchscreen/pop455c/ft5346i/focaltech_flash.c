...
static unsigned char CTPM_FW[] = {
	...
	//#include "firmware/POP4-5.5_4G_FT5346i_0x80_V13_D01_20150930_app.i"
	//#include "firmware/POP4-5.5_4G_FT5346i_0x80_V14_D01_20151125_app.i"
	//#include "firmware/POP4-5.5_4G_FT5346i_0x80_V15_D01_20151208_app.i"
	//#include "firmware/POP4-5.5_4G_FT5346i_0x80_V17_D01_20160108_app.i"
	//#include "firmware/POP4-5.5_4G_FT5346i_0x80_V18_D01_20160115_app.i"
	#include "firmware/POP4-5.5_4G_FT5346i_0x80_V1A_D01_20160121_app.i"
};
...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	dev_info(&client->dev, "fw vendor id = 0x%x\n", data->fw_vendor_id);
}
...
{	
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		/*********Step 3:check READ-ID***********************/
		printk("[FTS][%s] Step 3:check READ-ID \n",__func__);
		msleep(1);
		auc_i2c_write_buf[0] = FTS_READ_ID_REG;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] =0x00;		
		reg_val[0] = reg_val[1] = 0x00;
		...
		if(force_update){
			printk("[FTS] Step 3: READ OK CTPM ID,fts_updateinfo_curr.upgrade_id_1 = 0x%x,fts_updateinfo_curr.upgrade_id_2 = 0x%x\n",fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
	...
	if (i >= FTS_UPGRADE_LOOP )
		return -EIO;
	/*Step 4:erase app and panel paramenter area*/
	printk("[FTS][%s] Step 4:erase app and panel paramenter area \n", __func__);
	auc_i2c_write_buf[0] = FTS_ERASE_APP_REG;
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	msleep(1350);
	for(i = 0;i < 15;i++)
	...
	fts_i2c_write(client, auc_i2c_write_buf, 4);
	/*********Step 5:write firmware(FW) to ctpm flash*********/
	printk("[FTS][%s] Step 5:write firmware(FW) to ctpm flash \n",__func__);
	bt_ecc = 0;
	//FTS_DBG("Step 5:write firmware(FW) to ctpm flash\n");
	temp = 0;
	packet_number = (dw_lenth) / FTS_PACKET_LENGTH;
	...
	/*********Step 6: read out checksum***********************/
	/*send the opration head */
	printk("[FTS][%s]Step 6: read out checksum \n",__func__);
	auc_i2c_write_buf[0] = 0x64;
	fts_i2c_write(client, auc_i2c_write_buf, 1); 
	msleep(300);
...
int fts_ctpm_get_i_file_ver(void)
{
	...
	printk("[FTS]fts_updateinfo_curr.CHIP_ID = 0x%x\n",fts_updateinfo_curr.CHIP_ID);
	if (ui_sz > 2)
	{
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	if(force_update)
	{
		fts_updateinfo_curr.CHIP_ID = 0x54;
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	u8 uc_tp_fm_ver;
	int i_ret,i;
	unsigned char version_list[] = {0x00,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19};
	...
	for(i = 0; i < sizeof(version_list)/sizeof(version_list[0]); i++)
	{
		...
		printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x, version_list[%d] = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver,i,version_list[i]);
		if(uc_tp_fm_ver == version_list[i])
		{
			if(0x00 == uc_tp_fm_ver){
				force_update = true;
			}
			msleep(100);
