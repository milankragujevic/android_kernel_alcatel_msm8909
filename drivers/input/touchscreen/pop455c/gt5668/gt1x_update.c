...
int gt1x_update_judge(void)
{
	...
	u8 reg_val[2] = {0};
	u8 retry = 2;
	u16 update_list[] = {0x0119,0x0201};
	int update_list_len = sizeof(update_list)/sizeof(update_list[0]);
...
_reset:        
	...
	{
		if ((ver_info.patch_id & 0xFFFF) == update_list[i])
		{
			GTP_INFO("FW is need update!old=0x%x,new=0x%x", ver_info.patch_id, fw_ver_info.patch_id);
			return 0;
		}
	}
