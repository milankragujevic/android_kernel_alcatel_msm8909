...
#ifdef CONFIG_SENSORS_STK3X1X_TCT
...
static ssize_t gt1x_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	...
	if (gtp_suspend) {
		GTP_ERROR("Can not change gesture state in suspend %s.", buff);
		return -EPERM;
	}
               ...
               // GTP_ERROR("Gesture checksum error.");
                if (err_flag2) {
					if(0xcc == doze_buf[0]){
						...
						input_report_key(dev, KEY_GES_CUSTOM, 1);
						input_sync(dev);
						input_report_key(dev, KEY_GES_CUSTOM, 0);
						input_sync(dev);
					}
                    err_flag2 = 0;
                    ret = 0;
