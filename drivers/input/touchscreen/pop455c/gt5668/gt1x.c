...
static irqreturn_t gt1x_ts_irq_handler(int irq, void *dev_id)
{
	GTP_DEBUG_FUNC();
    	gt1x_irq_disable();
	wake_lock_timeout(&gtp_wake_lock, HZ/2);
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	wake_lock_init(&gtp_wake_lock, WAKE_LOCK_SUSPEND, "gtp-double-click");
...
#endif
	wake_lock_destroy(&gtp_wake_lock);
    	gt1x_deinit();
