...
static irqreturn_t mip_interrupt(int irq, void *dev_id)
{
    ...
    if(category == 0) {
        ...
        } else if(alert_type == MIP_ALERT_WAKEUP && gesture_wakeup_flag) {
            //Wake-up gesture
...
#else
/*Update fw from header file*/
int mip_fw_update_from_header_file(struct mip_ts_info *info)
{
    int retires = 3;
    int ret = fw_err_none;

    do {
        ret = mip_flash_fw(info, &melfas_mip4_fw[0], melfas_mip4_fw_len, false, true);
...
static int mip_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (g_ctp_is_founded == 1) {
        dev_info(&client->dev, "Already founded a right touch device!\n");
        return -1;
    }
    ...
    if (!info || !input_dev) {
        dev_err(&client->dev, "%s [ERROR] malloc mem!\n", __func__);
        ret = -ENOMEM;
        goto ERROR;
    }
...
#else
    ret = mip_fw_update_from_header_file(info);
#endif
    if(ret) {
