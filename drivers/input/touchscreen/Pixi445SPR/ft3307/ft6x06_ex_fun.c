...
static unsigned char CTPM_FW[] = {
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x12_20151130_app.i"
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x13_20151210_app.i"
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x15_20151223_app.i"
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x16_20160105_app.i"
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x17_20160120_app.i"
	//#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x18_20160126_app.i"
	#include "firmware/Pixi445SPR/PIXI4_45_TF_3307_TDT0xe0_Ver0x19_20160217_app.i"

};
static struct mutex g_device_mutex;
extern bool FT6xx6DebugMode ;

#define FT6336_ESD_PROTECT
#ifdef FT6336_ESD_PROTECT
extern int fts_esd_running ;
extern void ft6xx6_esd_switch(s32 on);
#endif

#define FT6XX6_TP_DBG(format, ...) \
{ \
    if(FT6xx6DebugMode) \
    { \
        printk(KERN_INFO "FT6x06 debug " format , ## __VA_ARGS__); \
    } \
}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	unsigned char version_list[] = {0x00,0x0a,0x0b,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18};
	...
	printk("TP project ID : 0x%x", tp_project_id);
   ...
   //other condition not try to update the firmware
	printk(KERN_ERR "The TP reg is %s,the project is PIXI3454GSPR \n",tp_project_reg);
					...
					char *buf)
{
	...
	if(vendor_id == 0x80) // Eachopto
	    ...
	    module_vendor = "Mutto";
	else if(vendor_id == 0xE0)
		module_vendor = "TDT"; //TCL 
	else
...
//Debug
ssize_t FT6xx6_Debug_Show(struct device *pDevice, struct device_attribute *pAttr, char *pBuf)
{

    printk("*** %s()***\n", __func__);

    return sprintf(pBuf, "%d\n", FT6xx6DebugMode);
}

ssize_t FT6xx6_Debug_Store(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize)
{
	if( pBuf[0] == '0')
	{
	  FT6xx6DebugMode  = false;
	}
	else if(pBuf[0]=='1')
	{
		FT6xx6DebugMode  = true;
	}
	else
	{
	...
	printk("*** %s() FT6xx6DebugMode = %d ***\n", __func__,FT6xx6DebugMode);

    return nSize;
}

static ssize_t ft6x06_ftstouchinfo_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
...
static ssize_t ft6x06_ftstouchinfo_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
#ifdef FT6336_ESD_PROTECT
static ssize_t ft6x06_ftstpesdcheck_show(struct device *pDevice, struct device_attribute *pAttr, char *pBuf)
{
    printk("*** %s()*** \n", __func__);
    return sprintf(pBuf, "%d\n", fts_esd_running);
}

/* echo 1 > ftstpesdcheck */
static ssize_t ft6x06_ftstpesdcheck_store(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize)
{
	if( pBuf[0] == '0')
	{
	 // fts_esd_running  = false;
	 ft6xx6_esd_switch(false);
	}
	else if(pBuf[0]=='1')
	{
		//fts_esd_running  = true;
		ft6xx6_esd_switch(true);
	}
	else
	{
	...
	printk("*** %s() fts_esd_running = %d ***\n", __func__,fts_esd_running);

    return nSize;
}
#endif 

static DEVICE_ATTR(debug, S_IRUGO | S_IWUSR, FT6xx6_Debug_Show,
			FT6xx6_Debug_Store);
...
* show fw info
*/
static DEVICE_ATTR(ftstouchinfo, S_IRUGO | S_IWUSR, ft6x06_ftstouchinfo_show,
			ft6x06_ftstouchinfo_store);

#ifdef FT6336_ESD_PROTECT
/*tp esd check control
*example:echo 1 > ftstpesdcheck
*/
static DEVICE_ATTR(ftstpesdcheck, S_IRUGO | S_IWUSR, ft6x06_ftstpesdcheck_show,
			ft6x06_ftstpesdcheck_store);
#endif
		...
		size_t count, loff_t *ppos)
{
	...
	FT6XX6_TP_DBG("*** %s *** proc_operate_mode = %d\n", __func__,proc_operate_mode);
   // printk("%s proc_operate_mode = %d\n",__func__,proc_operate_mode);
	...
	case PROC_READ_REGISTER:
		FT6XX6_TP_DBG("%s: PROC_READ_REGISTER \n", __func__);
		writelen = 1;
	...
	case PROC_WRITE_REGISTER:
		FT6XX6_TP_DBG("%s: PROC_READ_DATA \n", __func__);
		writelen = 2;
		ret = ft6x06_i2c_Write(client, writebuf + 1, writelen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:write iic error\n", __func__);
	...
	case PROC_WRITE_DATA:
		FT6XX6_TP_DBG("%s: PROC_READ_DATA/PROC_WRITE_DATA \n", __func__);
		writelen = count - 1;
		ret = ft6x06_i2c_Write(client, writebuf + 1, writelen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:write iic error\n", __func__);
