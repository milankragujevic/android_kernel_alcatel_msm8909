 ...
 *
 * @filename $filename$
 * @date     2015/10/27 15:23
 * @id       $id$
 * @version  1.3.1
 *
 * @brief
 * The core code of BMI120 device driver
 *
 * @detail
 * This file implements the core code of BMI120 device driver,
 * which includes hardware related functions, input device register,
 * device attribute files, etc.
*/

#include "bmi120.h"
#include "bmi120_driver.h"
...
#define BMI120_STORE_COUNT  (6000)
#define LMADA     (1)
...
/*! bmi120 acc/gyro calibration status in H/W layer */
...
static const int bmi_interrupt_type[] = {
	...
	BMI120_ANY_MOTION_X_ENABLE,
	BMI120_ANY_MOTION_Y_ENABLE,
	BMI120_ANY_MOTION_Z_ENABLE,
	BMI120_DOUBLE_TAP_ENABLE,
	BMI120_SINGLE_TAP_ENABLE,
	BMI120_ORIENT_ENABLE,
	BMI120_FLAT_ENABLE,
	/* Interrupt enable1, index=7~13*/
	BMI120_HIGH_G_X_ENABLE,
	BMI120_HIGH_G_Y_ENABLE,
	BMI120_HIGH_G_Z_ENABLE,
	BMI120_LOW_G_ENABLE,
	BMI120_DATA_RDY_ENABLE,
	BMI120_FIFO_FULL_ENABLE,
	BMI120_FIFO_WM_ENABLE,
	/* Interrupt enable2, index = 14~17*/
	BMI120_NOMOTION_X_ENABLE,
	BMI120_NOMOTION_Y_ENABLE,
	BMI120_NOMOTION_Z_ENABLE,
	//BMI120_STEP_DETECTOR_EN
};
...
struct bmi120_axis_data_t {
	s16 x;
	s16 y;
	s16 z;
};

struct bmi120_type_mapping_type {
...
static const struct bmi120_type_mapping_type sensor_type_map[] = {

	{SENSOR_CHIP_ID_BMI, SENSOR_CHIP_REV_ID_BMI, "BMI120/162AB"},
	{SENSOR_CHIP_ID_BMI_C2, SENSOR_CHIP_REV_ID_BMI, "BMI120C2"},
	{SENSOR_CHIP_ID_BMI_C3, SENSOR_CHIP_REV_ID_BMI, "BMI120C3"},

};

/*!bmi120 sensor time depends on ODR */
static const struct bmi_sensor_time_odr_tbl
	...
	{0x001000, 120000, 0x000fff},/*120ms, 6.25hz, odr_acc=4*/
	...
	{0x000010, 625, 0x00000f},/*0.625ms, 1200hz, odr=12*/
...
static void bmi_dump_reg(struct bmi_client_data *client_data)
{
	...
	client_data->device.bus_read(client_data->device.dev_addr,
			BMI120_USER_ACCEL_CONFIG_ADDR, dbg_buf1, REG_MAX1);
	dev_notice(client_data->dev, "\nFrom 0x40:\n");
...
/*!
* BMI120 sensor remapping function
...
static void bmi_remap_sensor_data(struct bmi120_axis_data_t *val,
...
static void bmi_remap_data_acc(struct bmi_client_data *client_data,
				struct bmi120_accel_t *acc_frame)
{
	struct bosch_sensor_data bsd;
...
static void bmi_remap_data_gyro(struct bmi_client_data *client_data,
					struct bmi120_gyro_t *gyro_frame)
{
	struct bosch_sensor_data bsd;
...
static int bmi_input_init(struct bmi_client_data *client_data)
{
	...
	dev_acc->name = SENSOR_NAME_ACC;
	dev_acc->id.bustype = BUS_I2C;

	input_set_capability(dev_acc, EV_ABS, ABS_MISC);
	input_set_abs_params(dev_acc, ABS_X, -65535, 65535, 0, 0);
	input_set_abs_params(dev_acc, ABS_Y, -65535, 65535, 0, 0);
	input_set_abs_params(dev_acc, ABS_Z, -65535, 65535, 0, 0);

	input_set_capability(dev_acc, EV_MSC, INPUT_EVENT_SGM);
	input_set_capability(dev_acc, EV_MSC, INPUT_EVENT_STEP_DETECTOR);
	input_set_capability(dev_acc, EV_MSC, INPUT_EVENT_FAST_ACC_CALIB_DONE);
	input_set_capability(dev_acc, EV_MSC, INPUT_EVENT_FAST_GYRO_CALIB_DONE);

	input_set_capability(dev_acc, EV_REL, REL_X);
	input_set_capability(dev_acc, EV_REL, REL_Y);
	input_set_capability(dev_acc, EV_REL, REL_Z);
	input_set_drvdata(dev_acc, client_data);

	err = input_register_device(dev_acc);
	if (err < 0) {
		input_free_device(dev_acc);
		dev_notice(client_data->dev, "bmi120 input free!\n");
		return err;
	}
	client_data->input_acc = dev_acc;
	dev_notice(client_data->dev,
		"bmi120 input register successfully, %s!\n",
		client_data->input_acc->name);


	dev_gyro = input_allocate_device();
	if (NULL == dev_gyro)
		return -ENOMEM;

	dev_gyro->name = SENSOR_NAME_GYRO;
	dev_gyro->id.bustype = BUS_I2C;

	input_set_capability(dev_gyro, EV_ABS, ABS_MISC);
	input_set_abs_params(dev_gyro, ABS_RX, -65535, 65535, 0, 0);
	input_set_abs_params(dev_gyro, ABS_RY, -65535, 65535, 0, 0);
	input_set_abs_params(dev_gyro, ABS_RZ, -65535, 65535, 0, 0);

	input_set_capability(dev_gyro, EV_MSC, INPUT_EVENT_SGM);
	input_set_capability(dev_gyro, EV_MSC, INPUT_EVENT_STEP_DETECTOR);
	input_set_capability(dev_gyro, EV_MSC, INPUT_EVENT_FAST_ACC_CALIB_DONE);
	input_set_capability(dev_gyro, EV_MSC, INPUT_EVENT_FAST_GYRO_CALIB_DONE);

	input_set_capability(dev_gyro, EV_REL, REL_X);
	input_set_capability(dev_gyro, EV_REL, REL_Y);
	input_set_capability(dev_gyro, EV_REL, REL_Z);
	input_set_drvdata(dev_gyro, client_data);

	err = input_register_device(dev_gyro);
	if (err < 0) {
		input_free_device(dev_gyro);
		dev_notice(client_data->dev, "bmi120 input free!\n");
		return err;
	}
	client_data->input_gyro = dev_gyro;
	dev_notice(client_data->dev,
		"bmi120 input register successfully, %s!\n",
...
#ifdef TWO_INPUT_BMI
	struct input_dev *dev_gyro;
	struct input_dev *dev_acc = client_data->input_acc;
	input_unregister_device(dev_acc);
	input_free_device(dev_acc);
	
	dev_gyro = client_data->input_gyro;
	input_unregister_device(dev_gyro);
	input_free_device(dev_gyro);
#else
...
static int bmi_check_chip_id(struct bmi_client_data *client_data)
{
	...
	u8 bmi_sensor_cnt = sizeof(sensor_type_map)
				/ sizeof(struct bmi120_type_mapping_type);
...
{	
	int err;
	struct bmi120_accel_t data_acc;
	struct bmi120_gyro_t data_gyro;
	struct bmi120_axis_data_t bmi120_udata;
	...
	//ACC
	err = BMI_CALL_API(read_accel_xyz)(&data_acc);
	if (err < 0)
		return;
	bmi120_udata.x = data_acc.x;
	bmi120_udata.y = data_acc.y;
	bmi120_udata.z = data_acc.z;
	
	input_report_abs(client_data->input_acc, ABS_X, bmi120_udata.y);
	input_report_abs(client_data->input_acc, ABS_Y, bmi120_udata.x);
	input_report_abs(client_data->input_acc, ABS_Z, bmi120_udata.z*-1);
	input_event(client_data->input_acc,EV_SYN,SYN_TIME_SEC,ktime_to_timespec(ts).tv_sec);
	input_event(client_data->input_acc,EV_SYN,SYN_TIME_NSEC,ktime_to_timespec(ts).tv_nsec);
	input_sync(client_data->input_acc);

	//GYRO
	err = BMI_CALL_API(read_gyro_xyz)(&data_gyro);
	if (err < 0)
		return;
	bmi120_udata.x = data_gyro.x;
	bmi120_udata.y = data_gyro.y;
	bmi120_udata.z = data_gyro.z;
	input_report_abs(client_data->input_gyro, ABS_RX, bmi120_udata.y);
	input_report_abs(client_data->input_gyro, ABS_RY, bmi120_udata.x);
	input_report_abs(client_data->input_gyro, ABS_RZ, bmi120_udata.z);
	input_event(client_data->input_gyro,EV_SYN,SYN_TIME_SEC,ktime_to_timespec(ts).tv_sec);
	input_event(client_data->input_gyro,EV_SYN,SYN_TIME_NSEC,ktime_to_timespec(ts).tv_nsec);
	input_sync(client_data->input_gyro);


	//printk("acc:x=%d,y=%d,z=%d :::::gyro: x=%d,y=%d,z=%d\n",
	//	data_acc.x,data_acc.y,data_acc.z,data_gyro.x,data_gyro.y,data_gyro.z);
...
static void bmi_work_func(struct work_struct *work)
{
	...
	bmi120_udata.x = data.x;
	bmi120_udata.y = data.y;
	bmi120_udata.z = data.z;

	bmi_remap_sensor_data(&bmi120_udata, client_data);
	/*report current frame via input event*/
	input_event(client_data->input, EV_REL, REL_X, bmi120_udata.x);
	input_event(client_data->input, EV_REL, REL_Y, bmi120_udata.y);
	input_event(client_data->input, EV_REL, REL_Z, bmi120_udata.z);
...
static ssize_t bmi120_chip_id_show(struct device *dev,
...
static ssize_t bmi120_err_st_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_sensor_time_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err = 0;
	u32 sensor_time;
...
static ssize_t bmi120_fifo_flush_store(struct device *dev,
...
static ssize_t bmi120_fifo_bytecount_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
...
static ssize_t bmi120_fifo_bytecount_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
...
int bmi120_fifo_data_sel_get(struct bmi_client_data *client_data)
{
	int err = 0;
...
static ssize_t bmi120_fifo_data_sel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err = 0;
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	err = bmi120_fifo_data_sel_get(client_data);
	if (err)
		return -EINVAL;
...
static ssize_t bmi120_fifo_data_sel_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
				...
				u8 *fifo_data, u16 fifo_length, char *buf)
{
	...
	struct bmi120_accel_t acc_frame_arr[FIFO_FRAME_CNT];
	struct bmi120_gyro_t gyro_frame_arr[FIFO_FRAME_CNT];
	struct bmi120_mag_xyzr_t mag_frame_arr[FIFO_FRAME_CNT];
	...
	for (i = 0; i < FIFO_FRAME_CNT; i++) {
		memset(&mag_frame_arr[i], 0, sizeof(struct bmi120_mag_xyzr_t));
		memset(&acc_frame_arr[i], 0, sizeof(struct bmi120_accel_t));
		memset(&gyro_frame_arr[i], 0, sizeof(struct bmi120_gyro_t));
	}
...
/*Acc Only*/
	...
	if (client_data->fifo_data_sel == BMI_FIFO_M_SEL) {
		...
		sensortime_duration_tbl[odr.mag_odr].ts_duration_us*LMADA;
			...
			//bmi120_bmm150_mag_compensate_xyz_raw(
			//&mag_comp_xyz, mag);
			bmi120_bmm150_mag_compensate_xyz(&mag_comp_xyz);
#endif
			len = sprintf(buf, "%s %d %d %d %u ",
						MAG_FIFO_HEAD,
						mag_comp_xyz.x,
...
static ssize_t bmi120_fifo_data_out_frame_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	...
	if (!err) {
		...
		err = bmi_burst_read_wrapper(client_data->device.dev_addr,
			BMI120_USER_FIFO_DATA__REG, g_fifo_data_arr,
...
static ssize_t bmi120_fifo_watermark_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
...
static ssize_t bmi120_fifo_watermark_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_fifo_header_en_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	unsigned char data = 0xff;
...
static ssize_t bmi120_fifo_header_en_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
...
static ssize_t bmi120_fifo_time_en_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
...
static ssize_t bmi120_fifo_time_en_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_fifo_int_tag_en_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err = 0;
	...
	fifo_tag_int = (fifo_tag_int1 << BMI120_INT0) |
			(fifo_tag_int2 << BMI120_INT1);
...
static ssize_t bmi120_fifo_int_tag_en_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	err += BMI_CALL_API(set_fifo_tag_intr1_enable)
			((fifo_tag_int_en & (1 << BMI120_INT0)) ? 1 :  0);
	err += BMI_CALL_API(set_fifo_tag_intr2_enable)
			((fifo_tag_int_en & (1 << BMI120_INT1)) ? 1 :  0);
...
static int bmi120_set_acc_op_mode(struct bmi_client_data *client_data,
...
static ssize_t bmi120_temperature_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	s16 temp = 0xff;
...
static ssize_t bmi120_place_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_delay_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
...
//[Feature]-BEGIN by TCTSZ.yongzhong.cheng@tcl.com,2015/9/16,for 538697:bmi120 getdata ok
#ifdef POWER_REGULATOR_BMI
#define BMI_VDD_MIN_UV	2600000
#define BMI_VDD_MAX_UV	3300000
#define BMI_VIO_MIN_UV	1800000
#define BMI_VIO_MAX_UV	1800000

static int bmi_power_ctl(struct bmi_client_data *data, bool on)
{
	int ret = 0;
	printk("func=%s,on=%d,power_enabled=%d\n",__func__,on,data->power_enabled);
	if (!on && data->power_enabled) {
		...
		dev_dbg(&data->client->dev, "bmi_power_ctl on=%d\n",
				on);
...
static int bmi_power_init(struct bmi_client_data *data, bool on)
{
	int ret;
...
reg_vdd_set:
	if (regulator_count_voltages(data->vdd) > 0)
		regulator_set_voltage(data->vdd, 0, BMI_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(data->vdd);
	return ret;
}
...
#ifdef CDEV_BMI
  static struct sensors_classdev sensors_bmi_acc_cdev = {
	 .name = SENSOR_NAME_ACC,
	 .vendor = "BOSCH",
	 .version = 1,
 ...
 int bmi_acc_enable_set(struct sensors_classdev *sensors_cdev,unsigned int enabled)
{    
	struct bmi_client_data *client_data = (struct bmi_client_data *)container_of(
		 sensors_cdev, struct bmi_client_data, bmi_acc_cdev);

	mutex_lock(&client_data->mutex_enable);
	if (enabled) {
			#ifdef POWER_REGULATOR_BMI
				bmi_power_ctl(client_data, true);
			#endif
			...
			#ifdef POWER_REGULATOR_BMI
				bmi_power_ctl(client_data, false);
			#endif
 ...
 int bmi_acc_poll_delay_set(struct sensors_classdev *sensors_cdev,unsigned int delay_msec)
{
	struct bmi_client_data *client_data = (struct bmi_client_data *)container_of(
				sensors_cdev, struct bmi_client_data, bmi_acc_cdev);
 ...
 static struct sensors_classdev sensors_bmi_gyro_cdev = {
	.name = SENSOR_NAME_GYRO,
	.vendor = "BOSCH",
	.version = 1,
  ...
  int bmi_gyro_enable_set(struct sensors_classdev *sensors_cdev,unsigned int enabled)
 {	  
	 struct bmi_client_data *client_data = (struct bmi_client_data *)container_of(
		  sensors_cdev, struct bmi_client_data, bmi_gyro_cdev);
  ...
  int bmi_gyro_poll_delay_set(struct sensors_classdev *sensors_cdev,unsigned int delay_msec)
 {
	 struct bmi_client_data *client_data = (struct bmi_client_data *)container_of(
				 sensors_cdev, struct bmi_client_data, bmi_gyro_cdev);
...
//yongzhong.cheng@tcl.com,2016/4/21,defect 1938927,add bmi pinctrl
#ifdef PINCTRL_BMI
static int bmi_pinctrl_init(struct bmi_client_data *data)
{
	...
	if (IS_ERR_OR_NULL(data->pinctrl)) {
		dev_err(data->dev, "Failed to look up default state\n");
		return PTR_ERR(data->pinctrl);
	}
...
static int bmi_pinctrl_state(struct bmi_client_data *client_data,bool active)
{
	int err=0;

	printk("bmi_pinctrl_state active=%d\n", active);
	if (active) {
	...
	if (err)
		dev_err(client_data->dev,"Error pinctrl_select_state sleep err:%d\n",err);
	return err;
}
...
static ssize_t bmi120_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	if (enable) {
		if (pre_enable == 0) {
			bmi120_set_acc_op_mode(client_data,
							BMI_ACC_PM_NORMAL);
			schedule_delayed_work(&client_data->work,
			msecs_to_jiffies(atomic_read(&client_data->delay)));
			atomic_set(&client_data->wkqueue_en, 1);
...
#if defined(BMI120_ENABLE_INT1) || defined(BMI120_ENABLE_INT2)
/* accel sensor part */
static ssize_t bmi120_anymot_duration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	unsigned char data;
...
static ssize_t bmi120_anymot_duration_store(struct device *dev,
...
static ssize_t bmi120_anymot_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_anymot_threshold_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_step_detector_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 data = 0;
	u8 step_det;
...
static ssize_t bmi120_step_detector_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_step_detector_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_signification_motion_enable_store(
	...
	const char *buf, size_t count)
{
	...
	if (data == 1) {
		err = BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_X_ENABLE, 1);
		err += BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_Y_ENABLE, 1);
		err += BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_Z_ENABLE, 1);
		if (err < 0)
			return -EIO;
		client_data->sig_flag = 1;
	} else {
		err = BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_X_ENABLE, 0);
		err += BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_Y_ENABLE, 0);
		err += BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_Z_ENABLE, 0);
		if (err < 0)
			return -EIO;
		client_data->sig_flag = 0;
	}
...
static ssize_t bmi120_signification_motion_enable_show(
...
static int sigmotion_init_interrupts(u8 sig_map_int_pin)
{
	int ret = 0;
/*0x60  */
	ret += bmi120_set_intr_any_motion_thres(0x1e);
/* 0x62(bit 3~2)	0=1.5s */
	ret += bmi120_set_intr_significant_motion_skip(0);
/*0x62(bit 5~4)	1=0.5s*/
	ret += bmi120_set_intr_significant_motion_proof(1);
/*0x50 (bit 0, 1, 2)  INT_EN_0 anymo x y z*/
	ret += bmi120_map_significant_motion_intr(sig_map_int_pin);
/*0x62 (bit 1) INT_MOTION_3	int_sig_mot_sel
close the signification_motion*/
	ret += bmi120_set_intr_significant_motion_select(0);
/*close the anymotion interrupt*/
	...
	ret += BMI_CALL_API(set_intr_enable_0)
					(BMI120_ANY_MOTION_Z_ENABLE, 0);
	if (ret)
		printk(KERN_ERR "bmi120 sig motion failed setting,%d!\n", ret);
	return ret;

}
#endif

static ssize_t bmi120_acc_range_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	unsigned char range;
...
static ssize_t bmi120_acc_range_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_acc_odr_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
...
static ssize_t bmi120_acc_odr_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
	...
	err = BMI_CALL_API(set_accel_output_data_rate)(acc_odr,2);
	if (err)
		return -EIO;
	client_data->odr.acc_odr = acc_odr;
	return count;
...
static ssize_t bmi120_acc_op_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_acc_op_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	err = bmi120_set_acc_op_mode(client_data, op_mode);
	if (err)
		return err;
	else
		return count;
...
static ssize_t bmi120_acc_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	...
	return sprintf(buf, "%hd %hd %hd\n",
			bmi120_udata.x, bmi120_udata.y, bmi120_udata.z);
}

static ssize_t bmi120_acc_fast_calibration_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_fast_calibration_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_acc_fast_calibration_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_fast_calibration_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_acc_fast_calibration_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_fast_calibration_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	#ifdef TWO_INPUT_BMI
		input_event(client_data->input_acc, EV_MSC,
...
static ssize_t bmi120_acc_offset_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_offset_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_acc_offset_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_offset_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_acc_offset_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_acc_offset_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	...
	err = client_data->device.bus_read(client_data->device.dev_addr,
			BMI120_USER_DATA_8_GYRO_X_LSB__REG, raw_data, 15);
	if (err)
		return err;
...
static ssize_t bmi120_step_counter_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_step_counter_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_step_counter_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_step_counter_clc_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_step_counter_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	s16 data;
	int err;
...
static ssize_t bmi120_bmi_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	...
	err = client_data->device.bus_read(client_data->device.dev_addr,
			BMI120_USER_DATA_8_GYRO_X_LSB__REG, raw_data, 12);
	if (err)
		return err;
...
static ssize_t bmi120_selftest_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
 ...
 */
static ssize_t bmi120_selftest_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	BMI120_ACCEL_OUTPUT_DATA_RATE_1200HZ,2);

	/* set to 8G range*/
	err += BMI_CALL_API(set_accel_range)(BMI120_ACCEL_RANGE_8G);
...
/* gyro sensor part */
static ssize_t bmi120_gyro_op_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi120_gyro_op_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
...
static ssize_t bmi120_gyro_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	struct bmi120_gyro_t data;
	struct bmi120_axis_data_t bmi120_udata;
	int err;
	...
	return sprintf(buf, "%hd %hd %hd\n", bmi120_udata.x,
				bmi120_udata.y, bmi120_udata.z);
}

static ssize_t bmi120_gyro_range_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	unsigned char range;
...
static ssize_t bmi120_gyro_range_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_gyro_odr_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
...
static ssize_t bmi120_gyro_odr_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_gyro_fast_calibration_en_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	int err;
...
static ssize_t bmi120_gyro_fast_calibration_en_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long enable;
...
static ssize_t bmi120_gyro_offset_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	s16 data = 0;
	s8 err = 0;
...
static ssize_t bmi120_gyro_offset_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_gyro_offset_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	s16 data = 0;
	s8 err = 0;
...
static ssize_t bmi120_gyro_offset_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_gyro_offset_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	s16 data = 0;
	int err = 0;
...
static ssize_t bmi120_gyro_offset_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
/* mag sensor part */
#ifdef BMI120_MAG_INTERFACE_SUPPORT
static ssize_t bmi120_mag_op_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	u8 mag_op_mode;
	s8 err;
	err = bmi120_get_mag_power_mode_stat(&mag_op_mode);
	if (err) {
		dev_err(client_data->dev,
			"Failed to get BMI120 mag power mode:%d\n", err);
		return err;
	} else
...
static ssize_t bmi120_mag_op_mode_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
...
#ifdef BMI120_AKM09912_SUPPORT
			err = bmi120_set_bst_akm_and_secondary_if_powermode
					(BMI120_MAG_FORCE_MODE);
#else
			err = bmi120_set_bmm150_mag_and_secondary_if_power_mode
					(BMI120_MAG_FORCE_MODE);
#endif
	...
	if (err) {
		dev_err(client_data->dev,
			"Failed to switch BMI120 mag power mode:%d\n",
			client_data->pw.mag_pm);
		return err;
	} else
		return count;
...
static ssize_t bmi120_mag_odr_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err = 0;
...
static ssize_t bmi120_mag_odr_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
...
static ssize_t bmi120_mag_i2c_address_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 data;
	s8 err;
...
static ssize_t bmi120_mag_i2c_address_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_mag_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	struct bmi120_mag_xyz_s32_t data;

	struct bmi120_axis_data_t bmi120_udata;
	int err;
	/* raw data with compensation */
#ifdef BMI120_AKM09912_SUPPORT
	err = bmi120_bst_akm09912_compensate_xyz(&data);
#else
	err = bmi120_bmm150_mag_compensate_xyz(&data);
#endif
...
static ssize_t bmi120_mag_offset_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err = 0;
...
static ssize_t bmi120_mag_offset_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static ssize_t bmi120_mag_chip_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	s8 err = 0;
	u8 mag_chipid;

	err = bmi120_set_mag_manual_enable(0x01);
	/* read mag chip_id value */
#ifdef BMI120_AKM09912_SUPPORT
	err += bmi120_set_mag_read_addr(AKM09912_CHIP_ID_REG);
		/* 0x04 is mag_x lsb register */
	err += bmi120_read_reg(BMI120_USER_DATA_0_MAG_X_LSB__REG,
							&mag_chipid, 1);

	/* Must add this commands to re-set data register addr of mag sensor */
	err += bmi120_set_mag_read_addr(AKM_DATA_REGISTER);
#else
	err += bmi120_set_mag_read_addr(BMI120_BMM150_CHIP_ID);
	/* 0x04 is mag_x lsb register */
	err += bmi120_read_reg(BMI120_USER_DATA_0_MAG_X_LSB__REG,
							&mag_chipid, 1);
	...
	err += bmi120_set_mag_read_addr(BMI120_BMM150_DATA_REG);
#endif

	err += bmi120_set_mag_manual_enable(0x00);
...
static DEVICE_ATTR(anymot_duration, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi120_anymot_duration_show, bmi120_anymot_duration_store);
static DEVICE_ATTR(anymot_threshold, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi120_anymot_threshold_show, bmi120_anymot_threshold_store);
static DEVICE_ATTR(std_stu, S_IRUGO,
		bmi120_step_detector_status_show, NULL);
static DEVICE_ATTR(std_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi120_step_detector_enable_show,
		bmi120_step_detector_enable_store);
static DEVICE_ATTR(sig_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi120_signification_motion_enable_show,
		bmi120_signification_motion_enable_store);
...
static DEVICE_ATTR(bmi_value, S_IRUGO,
		bmi120_bmi_value_show, NULL);


static struct attribute *bmi120_attributes[] = {
...
static struct attribute_group bmi120_attribute_group = {
	.attrs = bmi120_attributes
};
...
static void bmi_slope_interrupt_handle(struct bmi_client_data *client_data)
{
	...
	client_data->device.bus_read(client_data->device.dev_addr,
				BMI120_USER_INTR_STAT_2_ADDR, &status2, 1);
	anym_first[0] = BMI120_GET_BITSLICE(status2,
				BMI120_USER_INTR_STAT_2_ANY_MOTION_FIRST_X);
	anym_first[1] = BMI120_GET_BITSLICE(status2,
				BMI120_USER_INTR_STAT_2_ANY_MOTION_FIRST_Y);
	anym_first[2] = BMI120_GET_BITSLICE(status2,
				BMI120_USER_INTR_STAT_2_ANY_MOTION_FIRST_Z);
	anym_sign = BMI120_GET_BITSLICE(status2,
				...
				(struct bmi_client_data *client_data)
{
	...
	if (!err) {
		err = bmi_burst_read_wrapper(client_data->device.dev_addr,
			BMI120_USER_FIFO_DATA__REG, fifo_data,
...
#ifdef TWO_INPUT_BMI
	input_event(client_data->input_acc, EV_MSC, INPUT_EVENT_SGM, 1);
...
/*input_report_rel(client_data->input,INPUT_EVENT_SGM,1);*/
	input_sync(client_data->input);
#endif
	bmi120_set_command_register(CMD_RESET_INT_ENGINE);
...
static void bmi_irq_work_func(struct work_struct *work)
{
	...
	client_data->device.bus_read(client_data->device.dev_addr,
				BMI120_USER_INTR_STAT_0_ADDR, int_status, 4);

	if (BMI120_GET_BITSLICE(int_status[0],
	...
	if (BMI120_GET_BITSLICE(int_status[1],
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
...
#ifdef POWER_REGULATOR_BMI
	printk("bmi120 defined POWER_REGULATOR_BMI\n");
			err = bmi_power_init(client_data, true);
		if (err)
			goto exit_err_sysfs;
	
		err = bmi_power_ctl(client_data, true);
		if (err)
			goto exit_err_sysfs;
#else
	printk("bmi120 not defined POWER_REGULATOR_BMI\n");
#endif
	...
	/* sysfs node creation */
#ifdef TWO_INPUT_BMI
	err = sysfs_create_group(&client_data->input_acc->dev.kobj,
			&bmi120_attribute_group);
	err = sysfs_create_group(&client_data->input_gyro->dev.kobj,
			&bmi120_attribute_group);	
#else
...
#ifdef BMI120_MAG_INTERFACE_SUPPORT
	err += bmi120_set_command_register(MAG_MODE_NORMAL);
	mdelay(2);
	err += bmi120_get_mag_power_mode_stat(&mag_op_mode);
	mdelay(2);
	...
	mdelay(2);
#ifdef BMI120_AKM09912_SUPPORT
	err += BMI_CALL_API(set_i2c_device_addr)(BMI120_AKM09912_I2C_ADDRESS);
	/*do not need to check the return value for mag 2nd interface*/
	bmi120_bst_akm_mag_interface_init(BMI120_AKM09912_I2C_ADDRESS);
#else
	err += BMI_CALL_API(set_i2c_device_addr)(BMI120_AUX_BMM150_I2C_ADDRESS);
	/*do not need to check the return value for mag 2nd interface*/
	bmi120_bmm150_mag_interface_init(client_data->chip_id);
#endif
	err += bmi120_set_mag_burst(3);
	err += bmi120_get_mag_burst(&mag_urst_len);
	dev_info(client_data->dev,
		"BMI120 mag_urst_len:%d, mag_add:0x%x, mag_op_mode:%d\n",
	...
	if (err < 0)
		...
		dev_info(client_data->dev, "BMI120 qpio number:%d\n",
		...
		bmi120_set_intr_level(BMI_INT0, 1);
		/*set interrupt latch temporary, 5 ms*/
		/*bmi120_set_latch_int(5);*/

		BMI_CALL_API(set_output_enable)(
		BMI120_INTR1_OUTPUT_ENABLE, ENABLE);
		sigmotion_init_interrupts(BMI120_MAP_INTR1);
		BMI_CALL_API(map_step_detector_intr)(BMI120_MAP_INTR1);
		/*close step_detector in init function*/
		BMI_CALL_API(set_step_detector_enable)(0);
#endif

#ifdef BMI120_ENABLE_INT2
		/* maps interrupt to INT1/InT2 pin */
		BMI_CALL_API(set_intr_any_motion)(BMI_INT1, ENABLE);
		BMI_CALL_API(set_intr_fifo_wm)(BMI_INT1, ENABLE);
		//BMI_CALL_API(set_int_drdy)(BMI_INT1, ENABLE); //yongzhong.cheng temp

		/*Set interrupt trige level way */
		BMI_CALL_API(set_intr_edge_ctrl)(BMI_INT1, BMI_INT_LEVEL);
		bmi120_set_intr_level(BMI_INT1, 1);
		/*set interrupt latch temporary, 5 ms*/
		/*bmi120_set_latch_int(5);*/

		BMI_CALL_API(set_output_enable)(
		BMI120_INTR2_OUTPUT_ENABLE, ENABLE);
		sigmotion_init_interrupts(BMI120_MAP_INTR2);
		BMI_CALL_API(map_step_detector_intr)(BMI120_MAP_INTR2);
		/*close step_detector in init function*/
		BMI_CALL_API(set_step_detector_enable)(0);
#endif
		err = request_irq(client_data->IRQ, bmi_irq_handler,
				IRQF_TRIGGER_RISING, "bmi120", client_data);
		if (err)
	...
	if (bmi_pmu_set_suspend(client_data) < 0) {
		dev_err(dev, "Failed to set BMI120 to suspend power mode\n");
		goto exit_err_sysfs;
	}
	...
	#ifdef CDEV_BMI
		client_data->bmi_acc_cdev=sensors_bmi_acc_cdev;
		client_data->bmi_acc_cdev.sensors_enable = bmi_acc_enable_set;
		client_data->bmi_acc_cdev.sensors_poll_delay = bmi_acc_poll_delay_set;
		err = sensors_classdev_register(&client_data->input_acc->dev, &client_data->bmi_acc_cdev);
		if (err)
			goto exit_err_sysfs;

		client_data->bmi_gyro_cdev=sensors_bmi_gyro_cdev;
		client_data->bmi_gyro_cdev.sensors_enable = bmi_gyro_enable_set;
		client_data->bmi_gyro_cdev.sensors_poll_delay = bmi_gyro_poll_delay_set;
		err = sensors_classdev_register(&client_data->input_gyro->dev, &client_data->bmi_gyro_cdev);
		if (err)
			goto exit_err_sysfs;
	#endif
	
	#ifdef POWER_REGULATOR_BMI
		err = bmi_power_ctl(client_data, false);
		if (err)
			goto exit_err_sysfs;
	#endif
...
//yongzhong.cheng@tcl.com,2016/4/21,defect 1938927,add bmi pinctrl
	#ifdef PINCTRL_BMI
	if (bmi_pinctrl_init(client_data)) {
			printk("func=%s:bmi_pinctrl_init error\n",__func__);
		}else{
			err += bmi_pinctrl_state(client_data,true);
			if (err) {
...
exit_err_sysfs:
	if (err){
		...
		#ifdef POWER_REGULATOR_BMI
			//bmi_power_ctl(client_data, false);  //probe open ldo,suspend close it 
		#endif
		}
...
int bmi_remove(struct device *dev)
{
	...
	if (NULL != client_data) {
		...
		mdelay(5);
#ifdef TWO_INPUT_BMI
		sysfs_remove_group(&client_data->input_acc->dev.kobj,
				&bmi120_attribute_group);
		sysfs_remove_group(&client_data->input_gyro->dev.kobj,
				&bmi120_attribute_group);
...
static int bmi_post_resume(struct bmi_client_data *client_data)
{
	...
	if (atomic_read(&client_data->wkqueue_en) == 1) {
		bmi120_set_acc_op_mode(client_data, BMI_ACC_PM_NORMAL);
		schedule_delayed_work(&client_data->work,
				msecs_to_jiffies(
					atomic_read(&client_data->delay)));
	}
...
int bmi_suspend(struct device *dev)
{
	...
	if (atomic_read(&client_data->wkqueue_en) == 1) {
		bmi120_set_acc_op_mode(client_data, BMI_ACC_PM_SUSPEND);
		cancel_delayed_work_sync(&client_data->work);
	}
	BMI_CALL_API(get_step_counter_enable)(&stc_enable);
	BMI_CALL_API(get_step_detector_enable)(&std_enable);
	...
	if (client_data->pw.mag_pm != BMI_MAG_PM_SUSPEND) {
		...
		err += bmi120_set_bmm150_mag_and_secondary_if_power_mode
					(BMI120_MAG_SUSPEND_MODE);
#endif
		/*client_data->pw.gyro_pm = BMI120_MAG_SUSPEND_MODE;*/
		mdelay(3);
	}
