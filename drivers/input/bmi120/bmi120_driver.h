 ...
 *
 * @brief
 * The head file of BMI120 device driver core code
*/
#ifndef _BMI120_DRIVER_H
#define _BMI120_DRIVER_H
...
#define POWER_REGULATOR_BMI
#define CDEV_BMI
#define PINCTRL_BMI
#define TWO_INPUT_BMI


/* sensor specific */
#define SENSOR_NAME "bmi120"
#define SENSOR_NAME_ACC "bmi_acc"
#define SENSOR_NAME_GYRO   "bmi_gyro" 
...
#define BMI_REG_NAME(name) BMI120_##name##__REG
#define BMI_VAL_NAME(name) BMI120_##name
#define BMI_CALL_API(name) bmi120_##name
...
/*! bmi120 sensor spec of power mode */
struct pw_mode {
	u8 acc_pm;
	u8 gyro_pm;
	u8 mag_pm;
...
/*! bmi120 sensor spec of range */
struct range_t {
	u8 acc_range;
	u8 gyro_range;
};

/*! bmi120 sensor error status */
struct err_status {
	u8 fatal_err;
	u8 err_code;
	u8 i2c_fail;
...
/*! bmi120 fifo frame for all sensors */
struct fifo_frame_t {
	struct bmi120_accel_t *acc_farr;
	struct bmi120_gyro_t *gyro_farr;
	struct bmi120_mag_xyz_s32_t *mag_farr;
...
struct bmi_client_data {
	...
	struct sensors_classdev bmi_acc_cdev;
	struct sensors_classdev bmi_gyro_cdev;
