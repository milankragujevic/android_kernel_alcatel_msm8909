/* drivers/misc/akm0991x.c - akm0991x compass driver
 *
...
#define AKM0991x_VDD_MIN_UV	2000000
#define AKM0991x_VDD_MAX_UV	3300000
#define AKM0991x_VIO_MIN_UV	1750000
#define AKM0991x_VIO_MAX_UV	1950000
...
struct akm_compass_data {
	...
	int sensor_device_id;//***********
};

static struct sensors_classdev sensors_cdev_09911 = {
	.name = "akm09911-mag",
...
static struct sensors_classdev sensors_cdev_09916 = {
	.name = "akm09916-mag",
	.vendor = "Asahi Kasei Microdevices Corporation",
	.version = 1,
	.handle = SENSORS_MAGNETIC_FIELD_HANDLE,
...
/*****match the right device if akm09916 or akm09911********/
static int akm0991x_device_match(struct i2c_client *client)
{
	struct akm_compass_data *akm = i2c_get_clientdata(client);
	int err;
	akm->sense_info[0] = AK0991x_REG_WIA1;
	err = akm_i2c_rxdata(client, akm->sense_info, AKM_SENSOR_INFO_SIZE);
	if (err < 0)
		return err;

	if ((akm->sense_info[0] == AK0991x_WIA1_VALUE) &&
			(akm->sense_info[1] == AK0991x_WIA2_VALUE_09911)){
		//printk("%s: the device is AKM09911 Compass.\n", __func__);
		return 0;
	}
	else if ((akm->sense_info[0] == AK0991x_WIA1_VALUE) &&
			(akm->sense_info[1] == AK0991x_WIA2_VALUE_09916)){
		//printk("%s: the device is AKM09916 Compass.\n", __func__);
		return 1;
	}
	else {
		dev_err(&client->dev,"%s: not AKM Compass matching device failed.\n", __func__);
		return -1;
	}
}
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int err;
	uint8_t asa[3];
	int device_id = akm->sensor_device_id;
...
static int akm0991x_i2c_check_device(
	struct i2c_client *client)
{
	/* AK0991x specific function */
	struct akm_compass_data *akm = i2c_get_clientdata(client);
	int err;
	int device_id = akm->sensor_device_id;
	...
	err = AKECS_SetMode(akm, AK0991x_MODE_FUSE_ACCESS);
	if (err < 0)
		return err;

	if (device_id == 0) {
		akm->sense_conf[0] = AK0991x_FUSE_ASAX;
		err = akm_i2c_rxdata(client, akm->sense_conf, AKM_SENSOR_CONF_SIZE);
		if (err < 0)
			return err;
	}
	else if (device_id ==1) {
		akm->sense_conf[0] = 128;
		akm->sense_conf[1] = 128;
		akm->sense_conf[2] = 128;
	}
	else {
		return err;
	}
	
	err = AKECS_SetMode(akm, AK0991x_MODE_POWERDOWN);
	if (err < 0)
		return err;
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd,
				AKM0991x_VDD_MIN_UV, AKM0991x_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
		...
		if (regulator_count_voltages(data->vio) > 0) {
			rc = regulator_set_voltage(data->vio,
				AKM0991x_VIO_MIN_UV, AKM0991x_VIO_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
...
reg_vdd_set:
	if (regulator_count_voltages(data->vdd) > 0)
		regulator_set_voltage(data->vdd, 0, AKM0991x_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(data->vdd);
	return rc;
}
...
static int akm_report_data(struct akm_compass_data *akm)
{
	...
	if (device_id == 1) {
		input_report_abs(akm->input, ABS_MISC, mag_st2);
	}
...
static int akm_self_test(struct sensors_classdev *sensors_cdev)
{
	...
	asax = akm->sense_conf[AKM0991x_AXIS_X];
	asay = akm->sense_conf[AKM0991x_AXIS_Y];
	asaz = akm->sense_conf[AKM0991x_AXIS_Z];
	...
	case_test(akm, TLIMIT_TN_ASAX_0991x, asax, TLIMIT_LO_ASAX_0991x,
			TLIMIT_HI_ASAX_0991x, &fail_total);
	case_test(akm, TLIMIT_TN_ASAY_0991x, asay, TLIMIT_LO_ASAY_0991x,
			TLIMIT_HI_ASAY_0991x, &fail_total);
	case_test(akm, TLIMIT_TN_ASAZ_0991x, asaz, TLIMIT_LO_ASAZ_0991x,
			TLIMIT_HI_ASAZ_0991x, &fail_total);

	ret = AKECS_SetMode(akm, AK0991x_MODE_SNG_MEASURE);
	if (ret < 0) {
		dev_err(&akm->i2c->dev, "Set to single measurement failed.\n");
		goto exit;
	}

	count = AKM0991x_RETRY_COUNT;
	do {
	...
	hdata[AKM0991x_AXIS_X] = (s16)(i2c_data[1] | (i2c_data[2] << 8));
	hdata[AKM0991x_AXIS_Y] = (s16)(i2c_data[3] | (i2c_data[4] << 8));
	hdata[AKM0991x_AXIS_Z] = (s16)(i2c_data[5] | (i2c_data[6] << 8));

	i2c_data[0] &= 0x7F;
	case_test(akm, TLIMIT_TN_SNG_ST1_0991x,
	       (int)i2c_data[0], TLIMIT_LO_SNG_ST1_0991x,
		TLIMIT_HI_SNG_ST1_0991x, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_HX_0991x, hdata[0], TLIMIT_LO_SNG_HX_0991x,
			TLIMIT_HI_SNG_HX_0991x, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HY_0991x, hdata[1], TLIMIT_LO_SNG_HY_0991x,
			TLIMIT_HI_SNG_HY_0991x, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HZ_0991x, hdata[2], TLIMIT_LO_SNG_HZ_0991x,
			TLIMIT_HI_SNG_HZ_0991x, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_ST2_0991x, (int)i2c_data[8],
			TLIMIT_LO_SNG_ST2_0991x, TLIMIT_HI_SNG_ST2_0991x,
			&fail_total);

	/* self-test mode */
	ret = AKECS_SetMode(akm, AK0991x_MODE_SELF_TEST);
	if (ret < 0) {
	...
	case_test(akm, TLIMIT_TN_SLF_ST1_0991x, (int)i2c_data[0],
			TLIMIT_LO_SLF_ST1_0991x, TLIMIT_HI_SLF_ST1_0991x,
			&fail_total);
	...
	case_test(akm, TLIMIT_TN_SLF_RVHX_0991x, (hdata[0])*(asax/128 + 1),
			TLIMIT_LO_SLF_RVHX_0991x, TLIMIT_HI_SLF_RVHX_0991x,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHY_0991x, (hdata[1])*(asay/128 + 1),
			TLIMIT_LO_SLF_RVHY_0991x, TLIMIT_HI_SLF_RVHY_0991x,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHZ_0991x, (hdata[2])*(asaz/128 + 1),
			TLIMIT_LO_SLF_RVHZ_0991x, TLIMIT_HI_SLF_RVHZ_0991x,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_ST2_0991x, (int)i2c_data[8],
			TLIMIT_LO_SLF_ST2_0991x, TLIMIT_HI_SLF_ST2_0991x,
			&fail_total);
...
{
	struct akm0991x_platform_data *pdata;
	int err = 0;
	int i;
	...
	/***** check connection power operation******/
	err = akm_compass_power_init(s_akm, 1);
	if (err < 0)
		goto out_free_data;
	err = akm_compass_power_set(s_akm, 1);
	if (err < 0)
		goto out_power_deinit;
		

	/*******match the device id of akm compass**********/
	s_akm->sensor_device_id = akm0991x_device_match(client);
	if (s_akm->sensor_device_id < 0) {
		dev_err(&client->dev, "can't match the AKM compass device\n");
		goto out_power_exit;
	}

	/*******check device and set mode******/
	err = akm0991x_i2c_check_device(client);
	if (err < 0)
		goto out_power_exit;
	...
	if (err) {
		dev_err(&client->dev,"Failed to init pinctrl err=%d\n", err);
		goto out_power_exit;
	}
	if (!err) {
		err = pinctrl_select_state(s_akm->pinctrl, s_akm->pin_default);
		if (err) {
			dev_err(&client->dev, "Can't select pinctrl state\n");
			devm_pinctrl_put(s_akm->pinctrl);
			goto out_power_exit;
		}
	}
	#endif
	...
	if (s_akm->sensor_device_id == 0) {
		s_akm->cdev = sensors_cdev_09911;
	}
	else if (s_akm->sensor_device_id == 1) {
		s_akm->cdev = sensors_cdev_09916;
	}
	...
	s_akm->delay[MAG_DATA_FLAG] = sensors_cdev_09911.delay_msec * 1000000;

	err = sensors_classdev_register(&s_akm->input->dev, &s_akm->cdev);

	if (err) {
		dev_err(&client->dev, "class device create failed: %d\n", err);
		goto out_sensors_classdev_register;
	}
...
static struct of_device_id akm0991x_match_table[] = {
	{ .compatible = "ak,ak0991x", },
...
static struct i2c_driver akm_compass_driver = {
	...
	.driver = {
		.name	= AKM_I2C_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = akm0991x_match_table,
