...
#define AKM09916_VDD_MIN_UV	2000000
#define AKM09916_VDD_MAX_UV	3300000
#define AKM09916_VIO_MIN_UV	1750000
#define AKM09916_VIO_MAX_UV	1950000
...
static int akm09916_i2c_check_device(
	struct i2c_client *client)
{
	...
	err = AKECS_SetMode(akm, AK09916_MODE_FUSE_ACCESS);
	if (err < 0)
		return err;

	//akm->sense_conf[0] = AK09916_FUSE_ASAX;
	...
	err = AKECS_SetMode(akm, AK09916_MODE_POWERDOWN);
	if (err < 0)
		return err;
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd,
				AKM09916_VDD_MIN_UV, AKM09916_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
		...
		if (regulator_count_voltages(data->vio) > 0) {
			rc = regulator_set_voltage(data->vio,
				AKM09916_VIO_MIN_UV, AKM09916_VIO_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
...
reg_vdd_set:
	if (regulator_count_voltages(data->vdd) > 0)
		regulator_set_voltage(data->vdd, 0, AKM09916_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(data->vdd);
	return rc;
}
...
static int akm_self_test(struct sensors_classdev *sensors_cdev)
{
	...
	asax = akm->sense_conf[AKM09916_AXIS_X];
	asay = akm->sense_conf[AKM09916_AXIS_Y];
	asaz = akm->sense_conf[AKM09916_AXIS_Z];
	...
	case_test(akm, TLIMIT_TN_ASAX_09916, asax, TLIMIT_LO_ASAX_09916,
			TLIMIT_HI_ASAX_09916, &fail_total);
	case_test(akm, TLIMIT_TN_ASAY_09916, asay, TLIMIT_LO_ASAY_09916,
			TLIMIT_HI_ASAY_09916, &fail_total);
	case_test(akm, TLIMIT_TN_ASAZ_09916, asaz, TLIMIT_LO_ASAZ_09916,
			TLIMIT_HI_ASAZ_09916, &fail_total);

	ret = AKECS_SetMode(akm, AK09916_MODE_SNG_MEASURE);
	if (ret < 0) {
		dev_err(&akm->i2c->dev, "Set to single measurement failed.\n");
		goto exit;
	}

	count = AKM09916_RETRY_COUNT;
	do {
	...
	hdata[AKM09916_AXIS_X] = (s16)(i2c_data[1] | (i2c_data[2] << 8));
	hdata[AKM09916_AXIS_Y] = (s16)(i2c_data[3] | (i2c_data[4] << 8));
	hdata[AKM09916_AXIS_Z] = (s16)(i2c_data[5] | (i2c_data[6] << 8));

	i2c_data[0] &= 0x7F;
	case_test(akm, TLIMIT_TN_SNG_ST1_09916,
	       (int)i2c_data[0], TLIMIT_LO_SNG_ST1_09916,
		TLIMIT_HI_SNG_ST1_09916, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_HX_09916, hdata[0], TLIMIT_LO_SNG_HX_09916,
			TLIMIT_HI_SNG_HX_09916, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HY_09916, hdata[1], TLIMIT_LO_SNG_HY_09916,
			TLIMIT_HI_SNG_HY_09916, &fail_total);
	case_test(akm, TLIMIT_TN_SNG_HZ_09916, hdata[2], TLIMIT_LO_SNG_HZ_09916,
			TLIMIT_HI_SNG_HZ_09916, &fail_total);

	case_test(akm, TLIMIT_TN_SNG_ST2_09916, (int)i2c_data[8],
			TLIMIT_LO_SNG_ST2_09916, TLIMIT_HI_SNG_ST2_09916,
			&fail_total);

	/* self-test mode */
	ret = AKECS_SetMode(akm, AK09916_MODE_SELF_TEST);
	if (ret < 0) {
	...
	case_test(akm, TLIMIT_TN_SLF_ST1_09916, (int)i2c_data[0],
			TLIMIT_LO_SLF_ST1_09916, TLIMIT_HI_SLF_ST1_09916,
			&fail_total);
	...
	case_test(akm, TLIMIT_TN_SLF_RVHX_09916, (hdata[0])*(asax/128 + 1),
			TLIMIT_LO_SLF_RVHX_09916, TLIMIT_HI_SLF_RVHX_09916,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHY_09916, (hdata[1])*(asay/128 + 1),
			TLIMIT_LO_SLF_RVHY_09916, TLIMIT_HI_SLF_RVHY_09916,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_RVHZ_09916, (hdata[2])*(asaz/128 + 1),
			TLIMIT_LO_SLF_RVHZ_09916, TLIMIT_HI_SLF_RVHZ_09916,
			&fail_total);

	case_test(akm, TLIMIT_TN_SLF_ST2_09916, (int)i2c_data[8],
			TLIMIT_LO_SLF_ST2_09916, TLIMIT_HI_SLF_ST2_09916,
			&fail_total);
