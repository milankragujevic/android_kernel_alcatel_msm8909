...
struct bmp_client_data {
	...
	*/
	s8 selftest;
//[Feature]-BEGIN by TCTSZ.yongzhong.cheng@tcl.com,2016/2/17,for 1401030:bmp280 up raw date	
	struct sensors_classdev bmp_cdev;
//[Feature]-END by TCTSZ.yongzhong.cheng@tcl.com, 2016/2/17,for 1401030	
};
...
//[Feature]-BEGIN by TCTSZ.yongzhong.cheng@tcl.com,2016/2/17,for 1401030:bmp280 up raw date
int bmp_enable_set(struct sensors_classdev *sensors_cdev,unsigned int enabled)
{   
	struct bmp_client_data *data = (struct bmp_client_data *)container_of(
				sensors_cdev, struct bmp_client_data, bmp_cdev);

	printk("func=%s,enabled=%d\n",__func__,enabled);
...
int bmp_poll_delay_set(struct sensors_classdev *sensors_cdev,unsigned int delay_msec)
{
	struct bmp_client_data *data = (struct bmp_client_data *)container_of(
				sensors_cdev, struct bmp_client_data, bmp_cdev);
...
static struct sensors_classdev sensors_bmp_cdev = {
	.name = SENSOR_NAME,
	.vendor = "BOSCH",
	.version = 1,
	...
	.enabled = 0,
	.delay_msec = 200,
	.sensors_enable = bmp_enable_set,
	.sensors_poll_delay = bmp_poll_delay_set,
};
...
//[Feature]-BEGIN by TCTSZ.yongzhong.cheng@tcl.com,2016/2/17,for 1401030:bmp280 up raw date
	data->bmp_cdev=sensors_bmp_cdev;
	data->bmp_cdev.sensors_enable = bmp_enable_set;
	data->bmp_cdev.sensors_poll_delay = bmp_poll_delay_set;
	err = sensors_classdev_register(&data->input->dev, &data->bmp_cdev);
	if (err)
		goto exit_err_clean;
//[Feature]-END by TCTSZ.yongzhong.cheng@tcl.com, 2016/2/17,for 1401030
