...
#include "../linux-kernel-inc/config.h"
...
static void tfa98xx_enable_filter_interrupt(struct tfa98xx *tfa98xx);
...
enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	tfa98xx_enable_filter_interrupt(tfa98xx);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	 ...
	 * For TFA2 we should be able to load the profile without clock,
	 * currently we don't do it as a workaround (see ticket artf123859).
	 */
	mutex_lock(&tfa98xx->dsp_lock);
	tfa98xx_open(0);
	tfa98xx_dsp_system_stable(0, &ready);
...
static void tfa98xx_enable_filter_interrupt(struct tfa98xx *tfa98xx)
{
	...
	if (gpio_is_valid(tfa98xx->irq_gpio)) {
		dev_dbg(tfa98xx->codec->dev,
			"CLKD interrupt: enabled \n");

		/* update status_reg mask to match enabled interrupts */
		handles_local[tfa98xx->handle].interrupt_status[0] &=
					~TFA98XX_STATUSREG_CLKS;
		handles_local[tfa98xx->handle].interrupt_status[0] |=
					1 << TFA98XX_STATUSREG_CLKS_POS;

		/* update interrupt_reg to match enabled interrupts */
		handles_local[tfa98xx->handle].interrupt_enable[0] &=
					~TFA98XX_INTERRUPT_REG_CLKD;
		handles_local[tfa98xx->handle].interrupt_enable[0] |=
					0 << TFA98XX_INTERRUPT_REG_CLKD_POS;
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	/* Check for SPKS bit */
	if (val_ie & TFA98XX_STATUSREG_SPKS)
		tfa98xx_tapdet(tfa98xx);

	/* Check for CLKS bit (THIS IS REVERSED IN THE STATUS REG! WHEN INTERRUPT IS THERE THE CLKS STATUS IS 0 NOT 1!!) */
	if ((val_ie & TFA98XX_STATUSREG_CLKS) == 0) {
		pr_debug("interrupt: CLKS! \n");

		/*
		   checking ACS otherwise we close the device while a cold
		   start is still in progress
		*/
		if ((val & TFA98XX_STATUSREG_ACS) == 0) {
			tfa98xx_open(tfa98xx->handle);
			tfa_write_filters(tfa98xx->handle, tfa98xx_profile);
			tfa98xx_close(tfa98xx->handle);
		}
	}
}
