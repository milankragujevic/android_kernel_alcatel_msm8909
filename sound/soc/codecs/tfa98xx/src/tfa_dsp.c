...
static void tfa_set_query_info (int dev_idx) {
	...
	case 0x88:
		...
		break;
//Modify-Begin by TCTSZ. yuchao.pan@tcl.com, configure query info only for TFA9888UK, 20160114
#if 0
	case 0x97:
		/* tfa9897 */
	...
	case 0x12:
		...
		break;
#endif
//Modify-End by TCTSZ. yuchao.pan@tcl.com, configure query info only for TFA9888UK, 20160114
	default:
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		/* If the profile contains the .standby suffix go to pwoerdown
